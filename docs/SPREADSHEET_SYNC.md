// ==========================================
// VEKTORION GOOGLE APPS SCRIPT SYNC (V2 PRO)
// ==========================================
// Petunjuk Penggunaan:
// 1. Buka spreadsheet Kas Vektorion Anda.
// 2. Klik menu 'Ekstensi' > 'Apps Script'.
// 3. Hapus semua kode default dan ganti/paste dengan seluruh kode di bawah ini.
// 4. Pastikan APP_URL di bawah sudah sesuai dengan URL web Vektorion Anda.
//    - Gunakan URL DEVELOPMENT (ais-dev-...) saat sedang mengaktifkan container / mensinkronkan data.
//    - Gunakan URL PRODUCTION (ais-pre-...) hanya setelah web di-deploy / di-share secara final.
// 5. Simpan (klik ikon disket) lalu jalankan fungsi 'onOpen' atau muat ulang spreadsheet Anda.

// Gunakan URL development agar langsung terhubung ke container server aktif (PORT 3000)
const APP_URL = "https://ais-dev-zeq4q6g7mtj4ya4jl63pvs-56491373313.asia-southeast1.run.app";
// Alternatif jika di-share secara final: "https://ais-pre-zeq4q6g7mtj4ya4jl63pvs-56491373313.asia-southeast1.run.app"
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem("Cek Koneksi Ke Server", "testConnection")
    .addToUi();
}
...
function syncFormerMembersToFirestore(year) {
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    const rowYear = String(values[i][5] || "2026").trim() || "2026"; // Ambil tahun di Kolom F (index 5), default 2026
...
function calculateFormerMemberIncome(year) {
  ...
  for (let i = 1; i < data.length; i++) {
    const rowYear = String(data[i][5] || "2026").trim() || "2026"; // Ambil tahun di Kolom F (index 5)
    if (parseAmount(data[i][3]) > 0 && rowYear === String(year)) total += parseAmount(data[i][3]);
  }
  return total;
}
...
/**
 * Parsing nominal kas yang super pintar.
 * Dapat mengenali format nominal Indonesia (Rp 2.000, 1.5jt, dll) maupun English.
 */
function parseAmount(val) {
  ...
  if (s.indexOf("jt") !== -1) {
    mul = 1e6;
    s = s.replace("jt", "");
  } else if (s.indexOf("rb") !== -1 || s.indexOf("k") !== -1) {
    mul = 1e3;
    s = s.replace(/rb|k/g, "");
  }
...
/**
 * Menguji konektivitas antara Apps Script dengan Server Vektorion.
 */
function testConnection() {
  ...
  const options = {
    ...
    payload: JSON.stringify({
      ...
      data: { ping: "pong", time: new Date().toISOString() },
      method: "patch"
    }),
    muteHttpExceptions: true
  };
  ...
  try {
    ...
    const code = response.getResponseCode();
    if (code === 200) {
      SpreadsheetApp.getUi().alert("Koneksi SUKSES! Server Vektorion aktif dan siap melakukan sinkronisasi.");
    } else {
      SpreadsheetApp.getUi().alert("Koneksi GAGAL [" + code + "]: " + response.getContentText());
    }
  } catch (e) {
    SpreadsheetApp.getUi().alert("Koneksi GAGAL (Kritis): " + e.toString());
  }
}
